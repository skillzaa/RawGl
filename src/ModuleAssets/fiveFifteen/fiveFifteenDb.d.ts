import { ClrObj } from "../assets.js";
export default class FiveFifteenDb {
    private triangles;
    constructor();
    tri(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, clr: ClrObj): void;
    rectWH(x: number, y: number, width: number | undefined, height: number | undefined, clr: ClrObj): void;
    rect(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, clr: ClrObj): void;
    diagonal(y1: number | undefined, y2: number | undefined, lineWidth: number | undefined, clr: ClrObj): void;
    private uploadData;
}
//# sourceMappingURL=fiveFifteenDb.d.ts.map